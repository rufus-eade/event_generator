...
const CALENDAR_URLS = [
    ...
    {
        name: 'Member Events - NOT FOR PUBLIC SOCIALS',
        url: 'https://ics.teamup.com/feed/ksz6j1axw1d997esiy/15203161.ics',
        color: '#00a85a' // Fair Green
    }
];
...
// Parse title to extract organiser from parentheses
function parseTitleAndOrganiser(title) {
    const result = {
        title: title,
        organiser: null
    };
    
    const organiserMatch = title.match(/\(([^)]+)\)/);
    if (organiserMatch) {
        result.organiser = organiserMatch[1];
    ...
    for (const line of lines) {
        ...
        if (line === 'END:VEVENT') {
            if (currentEvent && (currentEvent.title || currentEvent.start)) {
                ...
                const parsedTitle = parseTitleAndOrganiser(currentEvent.title);
                currentEvent.title = parsedTitle.title;
                currentEvent.organiser = parsedTitle.organiser || calendarName;
...
// Format time range with organiser for display
function formatTimeRangeWithOrganiser(event) {
    const timeStr = formatTimeRange(event);
    const dayOfWeek = event.start ? event.start.toLocaleDateString('en-GB', { weekday: 'long' }) : '';
    
    if (event.organiser && event.organiser !== event.calendar) {
        return `${timeStr}, ${dayOfWeek} (${event.organiser})`;
    }
...
function renderEventsList() {
    ...
    sortedDays.forEach(dayKey => {
        ...
        eventsByDay[dayKey].forEach(({ event, index }) => {
            const timeStr = formatTimeRangeWithOrganiser(event);
...
function generateAltText() {
    ...
    eventsToInclude.forEach((event, index) => {
        ...
        if (dayOfWeek && timeStr) {
            altText += `When: ${dayOfWeek}, ${dateStr}, ${timeStr}\n`;
        } else if (dayOfWeek) {
            altText += `When: ${dayOfWeek}, ${dateStr}\n`;
        } else if (timeStr) {
            altText += `When: ${dateStr}, ${timeStr}\n`;
        } else {
            altText += `When: ${dateStr}\n`;
        }
        ...
        if (event.organiser && event.organiser !== event.calendar) {
            altText += `Organiser: ${event.organiser}\n`;
        }
        ...
        if (event.description && event.description.trim()) {
            ...
            // Remove any "Organiser:" at the beginning of the description
            description = description.replace(/^Organiser:\s*.+?(?:\n|$)/im, '');
